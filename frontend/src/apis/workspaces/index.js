...
export const updateWorkspaceRequest = async ({ workspaceId, updateData, token }) => {
    try {
        const response = await axios.put(`/workspaces/${workspaceId}`, 
            { workspaceId, name: updateData.name },
            {
                headers: {
                    'x-access-token': token
                }
...
export const addChannelRequest = async({ workspaceId, channelName, token}) => {
    try {
        ...
        console.log('API Request Error while creating a channel for the workspace: ', error);
        throw error.response?.data;
    }
};
...
export const addMemberToWorkspaceRequest = async ({ workspaceId, role, token }) => {
    try {
        ...
        console.log('API Request Error while adding the member to the workspace: ', error);
        throw error.response?.data;
    }
};
...
export const joinWorkspaceRequest = async ({workspaceId, joinCode, token}) => {
    try {
        ...
        console.log('API Request Error while joining the workspace: ', error);
        throw error.response?.data;
    }
};